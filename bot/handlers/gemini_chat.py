...
from utils.message_log import get_previous
...
# --------------------------------------------------------------------- #
# /resumen — resumir lo último que se habló, para el que vuelve después
# de un rato sin mirar el grupo.
# --------------------------------------------------------------------- #
DEFAULT_RESUMEN_COUNT = 150
MAX_RESUMEN_COUNT = 400  # tope real: es cuánto guarda utils/message_log.py por chat

_RESUMEN_PROMPT = (
    "Te paso la transcripción reciente de un grupo de Telegram, línea por línea como "
    "\"Nombre: mensaje\". Hacé un resumen breve, en viñetas, de los temas de los que se habló, "
    "quién dijo qué solo si es relevante para entender el tema, y si hubo alguna decisión o "
    "acuerdo. No inventes nada que no esté en la transcripción. Si hay mensajes de sistema, "
    "imágenes, stickers u otros mensajes sin texto útil, ignoralos. Respondé directamente con "
    "el resumen, sin introducciones tipo \"Acá tienes el resumen\".\n\n"
    "Transcripción:\n{transcript}"
)
...
async def resumen_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat is None or chat.type == ChatType.PRIVATE:
        await message.reply_text("Este comando se usa dentro de un grupo.")
        return

    if not settings.gemini_api_key and not settings.groq_api_key:
        await message.reply_text(
            "😅 Esta función necesita que el owner del bot configure GEMINI_API_KEY o "
            "GROQ_API_KEY en el .env. Avisale."
        )
        return

    db: Database = context.application.bot_data["db"]
    if not await db.is_group_activated(chat.id):
        await message.reply_text("Este grupo todavía no está activado con /activar.")
        return

    count = DEFAULT_RESUMEN_COUNT
    if context.args and context.args[0].isdigit():
        count = max(10, min(MAX_RESUMEN_COUNT, int(context.args[0])))

    stubs = get_previous(chat.id, message.message_id, count)
    stubs = [s for s in stubs if s.text]
    if not stubs:
        await message.reply_text(
            "No tengo mensajes recientes registrados todavía para resumir "
            "(el historial se guarda en memoria mientras el bot está corriendo, "
            "así que si se reinició hace poco, puede que todavía no haya nada)."
        )
        return

    transcript = "\n".join(f"{s.name}: {s.text}" for s in stubs)
    prompt = _RESUMEN_PROMPT.format(transcript=transcript)
