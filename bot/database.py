...
class Database:
    ...
    async def _run_migrations(self) -> None:
        ...
        await self._backfill_missing_group_titles()
        await self.conn.commit()

    async def _backfill_missing_group_titles(self) -> None:
        """Si known_groups.title quedó en NULL (el grupo se registró antes
        de que existiera esta lógica, o el bot ya no puede consultarlo en
        vivo porque lo sacaron de ahí), tratamos de recuperar el nombre
        real desde otras tablas donde SÍ quedó guardado en su momento:
        el registro de moderación, los reportes, y las verificaciones de
        captcha. Así, aunque el bot ya no tenga acceso al grupo, /grupos
        puede mostrar el nombre en vez de solo el ID."""
        await self.conn.execute(
            """
            UPDATE known_groups
            SET title = (
                SELECT group_name FROM mod_logs
                WHERE mod_logs.group_id = known_groups.group_id AND group_name IS NOT NULL AND group_name != ''
        ...
        await self.conn.execute(
            ...
            UPDATE known_groups
            SET title = (
                SELECT group_title FROM reports
                WHERE reports.group_id = known_groups.group_id AND group_title IS NOT NULL AND group_title != ''
                ORDER BY created_at DESC LIMIT 1
            )
            WHERE title IS NULL AND EXISTS (
        ...
        await self.conn.execute(
            ...
            UPDATE known_groups
            SET title = (
                SELECT group_title FROM captcha_state
                WHERE captcha_state.group_id = known_groups.group_id AND group_title IS NOT NULL AND group_title != ''
                ORDER BY created_at DESC LIMIT 1
            )
            WHERE title IS NULL AND EXISTS (
                SELECT 1 FROM captcha_state
                WHERE captcha_state.group_id = known_groups.group_id AND group_title IS NOT NULL AND group_title != ''
            )
            """
        )
